...
public class SNShapeRenderer implements Disposable {
    ...
    public SNShapeRenderer() {
        this.renderer = new ImmediateModeRenderer20(15000, false, true, 0);
    }
    ...
    public void mySegment(float x1, float y1, float x2, float y2, float thickness, float cosAngle, float sinAngle, boolean useGradient, Color c1, Color c2) {
        ...
        renderer.color(bits1); renderer.vertex(x1 + nx, y1 + ny, 0);
        renderer.color(bits1); renderer.vertex(x1 - nx, y1 - ny, 0);
        renderer.color(bits2); renderer.vertex(x2 - nx, y2 - ny, 0);
    ...
    public void mySegmentCurved(float x1, float y1, float x2, float y2, float thickness, float radius, Color c1, Color c2) {
        int segments = Math.max(2, (int) (Math.cbrt(Math.max(Math.hypot(x2 - x1, y2 - y1) * 0.5f, Math.abs(radius) * 0.5f)) * 8.0f));
        float dx = x2 - x1;
        float dy = y2 - y1;
        ...
        for (int i = 1; i <= segments; i++) {
            ...
            float curX = x1 + dx * t + perpX * (float) Math.sin(t * Math.PI);
            float curY = y1 + dy * t + perpY * (float) Math.sin(t * Math.PI);

            float segDx = curX - prevX;
            float segDy = curY - prevY;
            float segLen = (float) Math.hypot(segDx, segDy);
            float cosA = segLen == 0 ? 1.0f : segDx / segLen;
            float sinA = segLen == 0 ? 0.0f : segDy / segLen;

            mySegment(prevX, prevY, curX, curY, thickness, cosA, sinA, c2 != null, c1, c2);
            prevX = curX;
            prevY = curY;
        }
    }
    ...
    public void myTrapezoid(float x1, float y1, float x2, float y2, float t1, float t2, float cosAngle, float sinAngle, boolean useGradient, Color c1, Color c2) {
        ...
        renderer.color(bits1); renderer.vertex(x1 + nx1, y1 + ny1, 0);
        renderer.color(bits1); renderer.vertex(x1 - nx1, y1 - ny1, 0);
        renderer.color(bits2); renderer.vertex(x2 - nx2, y2 - ny2, 0);
